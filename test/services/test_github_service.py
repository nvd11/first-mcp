...
async def test_get_repositories():
    """
    Tests the get_repositories method of the GitHubService.
    """
    logger.info("Testing GitHubService.get_repositories...")
    ...
    try:
        ...
        assert len(repos) > 0
        assert len(repos) <= limit
        
        # Check structure of the first repo
        first_repo = repos[0]
        ...
        # Check if we can find 'py-github-agent' which we use in other tests
        repo_names = [r["name"] for r in repos]
        # Note: We can't guarantee 'py-github-agent' is in the top 5 if sorted by updated,
        # but we can at least print what we found.
        logger.success(f"Successfully fetched {len(repos)} repositories.")
        print(f"Repositories found: {repo_names}")
        
    except Exception as e:
        pytest.fail(f"get_repositories failed with an exception: {e}")
