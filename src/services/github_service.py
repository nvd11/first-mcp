...
class GitHubService:
    ...
    async def get_repositories(self, owner: str, limit: int = 100) -> List[Dict[str, Any]]:
        """
        异步获取指定用户的仓库列表。
        
        :param owner: GitHub 用户名或组织名
        :param limit: 返回的最大仓库数量
        :return: 仓库信息列表
        """
        ...
        try:
            ...
            logger.success(f"Successfully fetched {len(simplified_repos)} repositories for {owner}.")
